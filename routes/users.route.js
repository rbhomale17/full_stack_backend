...
// registration thing are working here
...
userRouter.post("/login",loginMiddleware,async (req,res)=>{
    ...
    try {
        ...
        const token = jwt.sign({ userID: user._id }, process.env.jwt_secret_key, { expiresIn: "4h" });//{ expiresIn: 60*60*4 } for foour hour
        res.send({msg:`Welcome!, ${user.name} login is Successfully.`,token:token,userId:user._id})
    } catch (error) {
        res.send({err:error.message})
    }
});
