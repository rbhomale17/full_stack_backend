...
const registrationMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (emailUser.length !== 0) res.send({ msg: "This Email is already registered. Try Log in" })
        else if (mobileuser.length !== 0) res.send({ msg: "This Mobile Number is already registered. Try Log in" })
        else {
